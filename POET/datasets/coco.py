...
import POET.datasets.transforms as T
from POET.util.kpts_ops import build_HSPR, build_SPR, COCO_CLASSES
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        keypoints[:, 0::3].clamp_(min=0, max=w) #MAA: setting max and min for width
        keypoints[:, 1::3].clamp_(min=0, max=h) #MAA: setting max and min for height
        keypoints[:, 2::3] = torch.where(keypoints[:, 2::3] == 0, keypoints[:, 2::3],
                                         keypoints[:, 2::3] - 1)  # COCO: o in (0,1,2). This makes 0,1 -> 0 and 2 -> 1
        #MAA: Visibility is 0 for not labelled or occluded, and 1 for visible
        ...
        if self.kpts_center == "center_of_mass":
            x_ctr = torch.Tensor(np.ma.average(keypoints[:, 0::3].clone(), axis=1, #MAA: x keypoints
                                            weights=keypoints[:, 2::3]).filled(0)) #MAA: weights are visibility of body parts
            y_ctr = torch.Tensor(np.ma.average(keypoints[:, 1::3].clone(), axis=1, #MAA: y keypoints
                                            weights=keypoints[:, 2::3]).filled(0)) #MAA: weights are visibility of body parts
            center = torch.stack((x_ctr, y_ctr, torch.ones(keypoints[:,2].shape)), dim=1) #MAA: Centers visiblities = 1
            for i in range(len(center)): # set center visibility of empty humans to 0
                #MAA: This will "go into the else statement of the Matcher and Kpts Loss function"
                if (center[i,0] and center[i,1]) == 0: #MAA: if center is x=0 and y=0
...
def make_coco_transforms(image_set, downsample, max_size=1333):
    ...
    # normalize = T.Compose([
    #     T.ToTensor(),
    #     T.Normalize([95.9855/255, 92.5986/255, 93.7941/255], [24.1763/255, 39.8265/255, 57.6776/255])
    # ])
    ...
    #         T.ToTensor(),
    #         T.RandomBrightness(brightness_range=(0.3, 4.0)),
    #         T.RandomContrast(contrast_range=(0.5, 5.0)),
    #         T.RandomSaturation(saturation_range=(0.3, 3.0)),
    #         T.Normalize([95.9855/255, 92.5986/255, 93.7941/255], [24.1763/255, 39.8265/255, 57.6776/255])
    #     ])
    ...
    normalize = T.Compose([
        T.ToTensor(),
        T.Normalize([52.85218037/255, 73.17263865/255, 44.53766757/255], [21.94960777/255, 41.2114908/255, 25.17313806/255])
    ])
