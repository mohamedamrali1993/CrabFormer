...
from POET.util import box_ops
from POET.util.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy, get_world_size, is_dist_avail_and_initialized) # @DETR removed interpolate import
#from util.misc import interpolate

from POET.models.backbone import build_backbone
from POET.models.matcher import build_matcher
# @DETR removed segmentation imports
from POET.models.transformer import build_transformer
...
class POET(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        self.kpt_embed = MLP(hidden_dim, hidden_dim, 11, 3)  # 53 = 17 keypoints(x,y,o) + center(x,y)
        # @DETR replaced DETR's bbox_embed with kpt_embed
    ...
    def forward(self, samples: NestedTensor):
        ...
        """
        # @DETR replaced pred_boxes with pred_kpts
        ...
        outputs_coord = self.kpt_embed(hs).sigmoid() # @DETR replaced bbox_embed with kpt_embed
        out = {'pred_logits': outputs_class[-1], 'pred_kpts': outputs_coord[-1]} # @DETR replaced pred_boxes with pred_kpts
        if self.aux_loss:
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord):
        ...
        return [{'pred_logits': a, 'pred_kpts': b} # @DETR changed pred_boxes to pred_kpts
...
class SetCriterion(nn.Module):
    ...
    """
    # @DETR assignment between ground truth keypoints instead of boxes
    ...
    def loss_labels(self, outputs, targets, indices, num_people, log=False): #MAA: Changed log to False
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_people]
        """
        # @DETR replaced nb_target_boxes with nb_people
    ...
    def loss_cardinality(self, outputs, targets, indices, num_people):
        # @DETR replaced num_boxes with num_people
    ...
    def loss_kpts(self, outputs, targets, indices, num_people):
        ...
        """
        # @DETR target dicts must contain the key of "keypoints" instead of boxes. target dicts contain a tensor of dim [num_people, 53] rather than [nb_target_boxes, 4]
        # @DETR target keypoints are expected normalized by the image size instead of target boxes expected in format (center_x, center_y, w, h), normalized by image size
        assert 'pred_kpts' in outputs # @DETR pred_kpts instead of pred_boxes
        idx = self._get_src_permutation_idx(indices)
        
        # @DETR this function is totally new to deal with keypoint behavior
        ...
        if all(target_kpts[:,2].clone().flatten()): # all centers are visible (e.g. center of mass)
            ...
            target_kpts = torch.stack(
                (target_kpts[:, 0::3].clone() * visible_mask, target_kpts[:, 1::3].clone() * visible_mask),
                                      dim=2).view(-1, 8)
            src_kpts = torch.stack((x_kpts * visible_mask, y_kpts * visible_mask), dim=2).view(-1, 8)
            ...
            target_kpts_abs = torch.stack(
                (target_kpts_abs[:, 0::3].clone() * visible_mask, target_kpts_abs[:, 1::3].clone() * visible_mask),
                                      dim=2).view(-1, 8)
            src_kpts_abs = torch.stack((x_kpts_abs * visible_mask, y_kpts_abs * visible_mask), dim=2).view(-1, 8)
        ...
        # @DETR detr loss is just 'loss_bbox' and 'loss_giou', whereas poet has 4 losses in its dict
        return losses

    # @DETR removed loss_masks function from detr
    ...
    def get_loss(self, loss, outputs, targets, indices, num_people, **kwargs): # @DETR num_boxes replaced with num_people
        loss_map = {
            'labels': self.loss_labels,
            'cardinality': self.loss_cardinality,
            'keypoints': self.loss_kpts # @DETR keypoints rather than 'boxes' and 'masks'
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_people, **kwargs) # @DETR num_people rather than num_boxes
    ...
    def forward(self, outputs, targets):
        ...
        num_people = torch.clamp(num_people / get_world_size(), min=1).item() # @DETR num people rather than num_boxes
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        """
        out_logits, out_kpts = outputs['pred_logits'], outputs['pred_kpts'] #extracting logits and keypoints from outputs

        assert len(out_logits) == len(target_sizes)
        assert target_sizes.shape[1] == 2

        prob = F.softmax(out_logits, -1) #FS: Applies the softmax function to the predicted logits to obtain probabilities
        scores, labels = prob[..., :-1].max(-1) #FS: Extracts the maximum value of the probabilities and the corresponding label, excluding the last class which is the no-object class
        ...
        # convert the center-relative positions to image-absolute positions FS: by adding the center coordinates
        out_kpts[..., 2::3] += out_kpts[..., 0].unsqueeze(-1)
        out_kpts[..., 3::3] += out_kpts[..., 1].unsqueeze(-1)

        # and from relative [0, 1] to absolute [0, height] coordinates
        #Scaling Keypoints to Image Size
        ...
        # @DETR results is a list of dicts with 'keypoints' instead of 'boxes'
        return results
...
def build(args):
    ...
    num_classes = 2 # @DETR num_classes is hard-coded to 2 instead of 20 or 91 or 250, depending on dataset
    ...
    losses = ['labels', 'keypoints', 'cardinality'] # @DETR losses has keypoints instead of boxes
